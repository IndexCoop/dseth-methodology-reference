...
export class AuctionRebalanceProposer {
  ...
  async getTargetWeights(nodeOperatorFactors: number[], hhiFactors: number[]): Promise<number[]> {
    ...
    const weightFactorSum = Object.keys(PoolIds).length + nodeOperatorFactors.reduce((a, b) => a + b) + hhiFactors.reduce((a, b) => a + b);
    for (let i = 0; i < Object.keys(PoolIds).length; i++) {
      targetWeights.push((1 + nodeOperatorFactors[i] + hhiFactors[i]) / weightFactorSum);
    }
    ...
    for (const poolId of Object.values(PoolIds)) {
      switch (poolId) {
        case PoolIds.Lido:
          ...
          const wstEthRateProvider = new Contract(stakingTokenRateProviders[poolId], WSTETH_ABI, this.signer);
          const stEthPerWstETH = await wstEthRateProvider.stEthPerToken();
          
          // Get ETH per stETH
          const stEthPriceFeed = "0x86392dC19c0b719886221c78AB11eb8Cf5c52812";
          const stEthRateProvider = new Contract(stEthPriceFeed, CHAINLINK_PRICE_FEED_ABI, this.signer);
        ...
        case PoolIds.Rocketpool:
          const rETHRateProvider = new Contract(stakingTokenRateProviders[poolId], RETH_ABI, this.signer);
          ethExchangeRates.push(await rETHRateProvider.getExchangeRate());
          break;
        case PoolIds.StakeWise:
          const osETHRateProvider = new Contract(stakingTokenRateProviders[poolId], OSETH_PRICE_FEED_ABI, this.signer);
          ethExchangeRates.push(await osETHRateProvider.getRate());
          break;
        case PoolIds.Frax:
          const sfrxETHRateProvider = new Contract(stakingTokenRateProviders[poolId], SFRXETH_ABI, this.signer);
          ethExchangeRates.push(await sfrxETHRateProvider.pricePerShare()); // Returns frxETH per sfrxETH, which is pegged to ETH price +/- 1%
          break;
        case PoolIds.Swell:
          const swETHRateProvider = new Contract(stakingTokenRateProviders[poolId], SWETH_ABI, this.signer);
          ethExchangeRates.push(await swETHRateProvider.getRate());
          break;
        case PoolIds.Stader:
          const ethXRateProvider = new Contract(stakingTokenRateProviders[poolId], ETHX_ABI, this.signer);
          ethExchangeRates.push(await ethXRateProvider.getExchangeRate());
          break;
        default:
          break;
      }
  ...
  async getProposeRebalanceParams(targetUnits: BigNumber[]): Promise<ProposeRebalanceParams> {
    ...
    for (const addr of Object.values(stakingTokenAddresses)) {
      if (oldComponents.includes(addr)) {
        ...
        const params = await this.getAuctionExecutionParamsForComponent(priceAdapter, this.auctionConfig, componentPricesInWei[i], targetUnits[i], false);
  ...
  getNodeOperatorWeightFactors(nodeOperatorCounts: number[]): number[] {
    const sqrtNodeOperatorTotal = nodeOperatorCounts.reduce((a,b) => a + Math.sqrt(b), 0);
    return nodeOperatorCounts.map((count) => Math.sqrt(count) / sqrtNodeOperatorTotal);
  }
  ...
  getHHIWeightFactors(protocolHHIScores: number[]) {
    const sumOfProtocolHHIScores = protocolHHIScores.reduce((a, b) => a + b, 0);
    return protocolHHIScores.map((score: number) => score / sumOfProtocolHHIScores);
  }
  ...
  async getNodeOperatorCounts(): Promise<number[]> {
    ...
    for (const poolId of Object.values(PoolIds)) {
      if (poolId === PoolIds.Stader) {
        const permissionedNodeOpCount = await this.ratedApi.getNodeOperatorCountForPool(this.ratedAccessToken, "Stader - Permissioned");
        const permissionlessNodeOpCount = await this.ratedApi.getNodeOperatorCountForPool(this.ratedAccessToken, "Stader - Permissionless");
        nodeOperatorCounts.push(permissionedNodeOpCount + permissionlessNodeOpCount);
      } else {
        const nodeOperatorCount = await this.ratedApi.getNodeOperatorCountForPool(this.ratedAccessToken, poolId);
  ...
  async getValidatorDistribution(): Promise<number[][]> {
    ...
    for (const poolId of Object.values(PoolIds)) {
      if (poolId === PoolIds.Frax) {
        ...
        const permissionedValidatorCounts = await this.getValidatorCountsByNodeOpForPool("Stader - Permissioned");
        const permissionlessValidatorCounts = await this.getValidatorCountsByNodeOpForPool("Stader - Permissionless");
        validatorDistribution.push(permissionedValidatorCounts.concat(permissionlessValidatorCounts));
      } else {
        validatorDistribution.push(await this.getValidatorCountsByNodeOpForPool(poolId));
      }
    }
  ...
  async getProtocolHHIScores(validatorDistribution: number[][]): Promise<number[]> {
    return validatorDistribution.map((nodeOpValidatorCounts: number[]) => {
      const totalValidatorsInPool = nodeOpValidatorCounts.reduce((a, b) => a + b, 0);
      const sumOfNodeOperatorHHI = nodeOpValidatorCounts.reduce(
        (a, nodeOpCount) => a + Math.pow(nodeOpCount / totalValidatorsInPool * 100, 2), 0
      );
  ...
  async getAuctionExecutionParamsForComponent(priceAdapter: Contract, config: AuctionConfig, priceInWei: BigNumber, targetUnit: BigNumber, isDecreasing: boolean): Promise<AuctionExecutionParams> {
    ...
    const maxPrice = priceInWei.mul(toWei(config.maxPriceAsPercentOfMarketPrice));
    const minPrice = priceInWei.mul(toWei(config.minPriceAsPercentOfMarketPrice));
  ...
  async getValidatorCountsByNodeOpForPool(poolId: string): Promise<number[]> {
    ...
    let page = await this.ratedApi.getPaginatedNodeOperators(this.ratedAccessToken, poolId, size, from);
